...
# Restaurant & Travel Concierge Meal Generator
# -------------------------------------------
# A single-file Flask app + headless CLI that:
# - Accepts TDEE directly or estimates it from stats
# - Applies a 25% deficit for weight loss by default
# - Targets high-protein using either g/lb or % of calories
# - Builds a daily plan from chain/cuisine items
# - Exports HTML/JSON/**PDF** artifacts in headless mode
#
# Run (UI):
...
#   python concierge_app.py --headless --tdee 2400 --goal loss25 --weight_lb 200 --days 3 --out artifacts
#
# Notes:
# - This ships with a small seed dataset (chains & items). You can expand MENU below or wire a DB.
# - PDF export uses ReportLab. If it's not installed, you'll get a helpful error message.

from __future__ import annotations
import os, json, random, argparse, math, datetime, pathlib, textwrap, tempfile
...
# Allow embedding this app inside your GHL membership domain
# Replace the example domains with YOUR actual membership domain(s)
@app.after_request
def add_embed_headers(resp):
    # Example GHL domains (edit to your own): 
    # - your custom membership subdomain (e.g., https://members.yourdomain.com)
    # - (If you’re using a default GHL/myleadconnector domain, put that one)
    resp.headers["Content-Security-Policy"] = (
        "frame-ancestors 'self' https://members.activhustle.com"
    )
    # X-Frame-Options is legacy; CSP is the modern control. Omit or set SAMEORIGIN.
    return resp
...
# -----------------------------
# Fields: name, chain, cuisine, K (kcal), P (protein g), C (carb g), F (fat g)
...
def protein_from_strategy(calories: int,
    ...
    if strategy == "per_lb" and body_weight_lb:
        ...
        g = min(g, 1.2 * body_weight_lb)  # cut-friendly cap
        return int(round(g))
...
def score_combo(picks: List[Dict[str, Any]], cal_target: int, p_target: int, c_target: int, f_target: int) -> Tuple[float, Dict[str, float]]:
    K = sum(x["K"] for x in picks)
    P = sum(x["P"] for x in picks)
    C = sum(x["C"] for x in picks)
    F = sum(x["F"] for x in picks)

    # stronger adherence to calories + protein-first
    cal_pen = abs(K - cal_target) / 6.0
    prot_short = max(0, p_target - P)
    prot_pen = prot_short * 8.5

    # soft penalties for big macro overshoots (but don't overconstrain)
    fat_overshoot = max(0, F - f_target * 1.25)
    carb_overshoot = max(0, C - c_target * 1.35)
    macro_pen = (fat_overshoot * 0.7) + (carb_overshoot * 0.3)

    # small reward for variety of chains
    chains = {x["chain"] for x in picks}
    variety_bonus = -0.4 * (len(chains) - 1)
...
def macro_targets(calories: int, protein_g: Optional[int] = None) -> Tuple[int, int, int]:
    """Return (protein_g, carbs_g, fat_g) targets. Protein can be override."""
    if protein_g is None:
        protein_g = int(round(0.35 * calories / 4))  # default 35% protein
...
def pick_combo(pool: List[Dict[str, Any]], cal_target: int, meals_per_day: int, p_target: int, c_target: int, f_target: int) -> Tuple[List[Dict[str, Any]], Dict[str, float]]:
    ...
    for _ in range(TRIES):
        k = min(meals_per_day, len(top_pool))
        picks = random.sample(top_pool, k=k)
    ...
    if best_meta and abs(best_meta.get("K", 0) - cal_target) > 125:
        for _ in range(1500):
            ...
            if s < best_score:
                best_score, best_combo, best_meta = s, picks, meta
                if abs(best_meta["K"] - cal_target) <= 125:
                    break
...
def filter_menu(menu: List[Dict[str, Any]], cuisine: Optional[str], chain: Optional[str]) -> List[Dict[str, Any]]:
    out = menu
    if cuisine:
        out = [m for m in out if m["cuisine"].lower().startswith(cuisine.lower())]
    if chain:
        out = [m for m in out if m["chain"].lower() == chain.lower()]
    return out if out else menu

def generate_plan(calories: int,
                  cuisine: Optional[str],
                  chain: Optional[str],
                  days: int = 3,
                  protein_g_override: Optional[int] = None,
                  meals_per_day: int = DEFAULT_MEALS_PER_DAY) -> Dict[str, Any]:
    ...
    return {
        ...
        "carb_target": c_target,
        "fat_target": f_target,
        "plan": [{
            "items": d.items,
            "K": d.K, "P": d.P, "C": d.C, "F": d.F
        } for d in plan_days]
    }
...
.meal{padding:10px 0; border-top:1px dashed #2a3647}
...
<div class="container">
  ...
  <div class="muted small">Tip: you can override with <span class="kbd">calories=1800</span> directly in the URL.</div>
</div>
"""
...
def write_pdf_plan(path: str, plan: Dict[str, Any], calories: int):
    ...
    # Draw a simple colored bar header via Table
    header_tbl = Table(
        [[Paragraph(title, styles["H1"])]],
        colWidths=[6.5*inch]
    )
    header_tbl.setStyle(TableStyle([
        ("BACKGROUND", (0,0), (-1,-1), colors.HexColor("#68b5ff")),
    ...
    for idx, d in enumerate(plan["plan"], start=1):
        ...
        for it in d["items"]:
            data_rows.append([
                ...
                Paragraph(f"{it['chain']} • {it['cuisine']}", styles["Cell"]),
        ...
        tbl.setStyle(TableStyle([
            ("BACKGROUND", (0,0), (-1,0), colors.HexColor("#131820")),
            ("TEXTCOLOR", (0,0), (-1,0), colors.white),
            ("LINEBELOW", (0,0), (-1,0), 1, colors.HexColor("#0b0e11")),
        ...
        if idx % 3 == 0 and idx != len(plan["plan"]):
            story.append(PageBreak())

    story.append(Spacer(1, 12))
    story.append(Paragraph(f"Generated {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}", styles["Meta"]))
...
def _resolve_from_request(req):
    calories_param = req.get("calories")
    tdee = req.get("tdee")
    goal = req.get("goal", "loss25")

    sex = req.get("sex")
    weight_lb = req.get("weight_lb")
    height_in = req.get("height_in")
    age = req.get("age")
    activity = req.get("activity", "moderate")

    cuisine = req.get("cuisine") or None
    chain = req.get("chain") or None
    days = int(req.get("days", 3))

    prot_strategy = req.get("protein_strategy", "per_lb")
    prot_percent  = float(req.get("protein_percent", 0.35))
    prot_per_lb   = float(req.get("protein_per_lb", 0.9))

    meta_note = None
    try:
        if calories_param:
            calories_resolved = int(calories_param)
            meta_note = f"Using explicit calories = {calories_resolved}"
        else:
            if tdee:
                tdee = int(tdee)
            else:
                if all([sex, weight_lb, height_in, age]):
                    tdee = calc_tdee_from_stats(
                        sex=sex,
                    ...
                    tdee = 2000
                    meta_note = "Fallback: TDEE defaulted to 2000"
            calories_resolved = calorie_goal_from_tdee(int(tdee), goal)
            if not meta_note:
                meta_note = f"TDEE {tdee} → goal '{goal}' ⇒ {calories_resolved} kcal/day"
    except Exception:
        calories_resolved = 2000
        meta_note = "Invalid inputs; defaulted to 2000 kcal"

    try:
        bw = float(weight_lb) if weight_lb else None
        protein_g = protein_from_strategy(
            calories=calories_resolved,
            strategy=prot_strategy,
            body_weight_lb=bw,
            percent=prot_percent,
            per_lb=prot_per_lb
        )
    except Exception:
        protein_g = None
...
""",
        title="Concierge Meal Generator — Seed",
        items=items,
        n=len(items)
    )
        ...
        return render_template_string(
            ...
            {% block content %}
              <div class="card">
                ...
                <p class="small">Install ReportLab first:</p>
                <pre class="small">pip install reportlab</pre>
                <a class="underline" href="{{ url_for('index') }}">Back</a>
              </div>
            {% endblock %}
            """,
            title=f"{APP_NAME} — PDF Export",
            base=BASE_HTML
        ), 501
...
def write_html_plan(path: str, plan: Dict[str, Any], calories: int):
    """Headless-friendly HTML writer (no Flask context needed)."""
    # Minimal, self-contained HTML (dark header, simple tables).
    style = """
    <style>
      ...
      th{background:#131820; color:#ffffff; text-align:left;}
    </style>
    """
    header = f"<div class='hdr'>Concierge Meal Generator — {__import__('datetime').datetime.now().strftime('%Y-%m-%d')}</div>"
    ...
    parts.append(f"<div class='meta'>Generated {__import__('datetime').datetime.now().strftime('%Y-%m-%d %H:%M')}</div>")
    html = "\n".join(parts)
...
def headless_export(calories: int, cuisine: Optional[str], chain: Optional[str], days: int, protein_g: Optional[int], out_dir: str) -> Dict[str, str]:
    ...
    stamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    base = out / f"concierge_plan_{stamp}"
    files = {
    ...
    if REPORTLAB_AVAILABLE:
        write_pdf_plan(files["pdf"], plan, calories)
    else:
        # Write a tiny sentinel file so users know why PDF is missing
...
def main_cli():
    ...
    parser.add_argument("--debug", action="store_true", help="Run Flask debug server")

    # Headless flags
    parser.add_argument("--headless", action="store_true", help="Generate artifacts without running server")
    ...
    parser.add_argument("--goal", type=str, default="loss25")  # loss25 | maintain | gain10

    # Stats to estimate TDEE
    ...
    # Protein strategy
    parser.add_argument("--protein_strategy", type=str, default="per_lb")
    parser.add_argument("--protein_percent", type=float, default=0.35)
    parser.add_argument("--protein_per_lb", type=float, default=0.9)
    ...
    # Resolve calories for headless mode
    if args.calories:
    ...
    # Resolve protein target
    try:
        protein_resolved = protein_from_strategy(
            calories=calories_resolved,
            strategy=args.protein_strategy,
            body_weight_lb=args.weight_lb,
            percent=args.protein_percent,
            per_lb=args.protein_per_lb
        )
    except Exception:
        protein_resolved = None
